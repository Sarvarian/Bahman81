...
public class TestGrid2D : ClassTestBase
{
    ...
    [Fact]
    public void GameWorldIsNull()
    {
        Assert.Null(grid_.World);
    }

    [Fact]
    public void LocationToPositionConsideringOffset()
    {
        var offset = RandomVector2I(130, 140, 160, 170);
        Assert.NotEqual(Vector2.Zero, offset);
        grid_.World = new GameWorld(offset);
        var randomLocation = 0;
        while (randomLocation == 0) { randomLocation = Rng.Next(-10, 10); }
        var expectedResult = new Vector2
        {
            X = (randomLocation * grid_.CellSize.X) + offset.X,
            Y = offset.Y
        };
        Assert.Equal(expectedResult, grid_.LocationToPosition(randomLocation));
    }
    ...
    [Fact]
    public void Vector2ILocationToPositionConsideringOffset()
    {
        var offset = RandomVector2I(130, 140, 160, 170);
        Assert.NotEqual(Vector2.Zero, offset);
        grid_.World = new GameWorld(offset);
        ...
        var expected = new Vector2
        {
            X = (randomLocation.X * grid_.CellSize.X) + offset.X,
            Y = (randomLocation.Y * grid_.CellSize.Y) + offset.Y
        };
        Assert.Equal(expected, grid_.LocationToPosition(randomLocation));
    }

    [Fact]
    public void PositionToLocationConsideringOffset()
    {
        var offset = RandomVector2I(130, 140, 160, 170);
        Assert.NotEqual(Vector2.Zero, offset);
        grid_.World = new GameWorld(offset);
        var position = RandomVector2(10.0f, 20.0f, 40.0f, 50.0f);
        var positionWithoutOffset = position - offset;
        var locFloat = positionWithoutOffset / (Vector2)grid_.CellSize;
        var locInt = locFloat.Round().ToVec2I();
        var expected = locInt;
        Assert.Equal(expected, grid_.PositionToLocation(position));
    }
